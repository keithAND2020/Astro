...
@MODEL.register
class CFSR(Base_Model):
    def __init__(self, in_chans=3, embed_dim=96, depths=(6, 6, 6, 6), dw_size=3,
                 mlp_ratio=4., upscale=2, img_range=1., upsampler='',**kwargs):
        super(CFSR, self).__init__(**kwargs)
...
if __name__ == '__main__':
    ...
    model = CFSR(in_chans=1, embed_dim=48, depths=(6,6), dw_size=3,
                 mlp_ratio=2, upscale=2, img_range=1., upsampler='pixelshuffledirect').cuda()
    # print(model)
    ...
    x = model(x)
    print(x.shape)#UDA_VISIBLE_DEVICES=0class 