...
# 2. 应用PSF、噪声和下采样的函数
def degrade_patch(image, mask, scale_factor, noise_level_range=[0, 5]):
    nan_mask = mask
    ...
    #sigma_range = [0.5, 2]
    sigma_range = [0.2 * scale_factor, 1 * scale_factor]
    ...
    blurred_image = convolve(image_no_nan, psf, normalize_kernel=True)

    #blurred_image[~nan_mask] = np.nan
