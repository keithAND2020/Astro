...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection='1conv', **kargs):
        ...
        # 如果后续改了目标图片的通道数，这里一定要改，但是一般也就是1吧
        self.cross_attn = Cross_MultiAttention(in_channels=1, emb_dim=dim, num_heads=num_heads)
    ...
    def forward(self, target, x, x_size):
    # def forward(self, x, x_size):
        swin_attn_x = self.residual_group(x, x_size)
        attn_x = self.cross_attn(target, swin_attn_x)
        return self.patch_embed(self.conv(self.patch_unembed(attn_x, x_size))) + x
...
class Cross_MultiAttention(nn.Module):
    def __init__(self, emb_dim, num_heads, in_channels=1, att_dropout=0.0, aropout=0.0):
...
class SwinIR(nn.Module):
    ...
    def forward_features(self, target, x):
    ...
    def forward(self, x):
        target_signal_q = x[:, 0, :, :].unsqueeze(1)
        x = x[:, 1, :, :].unsqueeze(1)
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(target_signal_q, x)) + x
            ...
            res = self.conv_after_body(self.forward_features(target_signal_q, x_first)) + x_first
...
if __name__ == '__main__':
    ...
    model = SwinIR(upscale=1, in_chans=1,img_size=(height, width),
